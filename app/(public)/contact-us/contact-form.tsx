...
export function ContactForm() {
  ...
  if (status === "success") {
    return (
      <div className={styles.formSuccess} role="status" aria-live="polite" tabIndex={-1}>
        ...
        soon as possible. For account-specific questions, use the in-app support channel.
      </div>
    );
  }
  ...
  return (
    <form ref={formRef} className={styles.form} onSubmit={handleSubmit} noValidate aria-label="Contact form">
      {/* Error summary */}
      {hasErrors && (
        <div ref={errorSummaryRef} role="alert" aria-live="assertive" tabIndex={-1}
          style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 6, padding: "14px 18px", color: "#991b1b", fontSize: 14 }}>
      ...
      <label htmlFor="cf-name">
        ...
        {errors.name && (
          <span id="cf-name-error" className={styles.fieldError} role="alert">{errors.name}</span>
        )}
      </label>
      ...
      <label htmlFor="cf-email">
        ...
        <input id="cf-email" name="email" type="email" autoComplete="email" required
          ...
          disabled={status === "loading"} />
        {errors.email && (
          <span id="cf-email-error" className={styles.fieldError} role="alert">{errors.email}</span>
        )}
      </label>
      ...
      <label htmlFor="cf-subject">
        ...
        <input id="cf-subject" name="subject" type="text" required
          ...
          disabled={status === "loading"} />
        {errors.subject && (
          <span id="cf-subject-error" className={styles.fieldError} role="alert">{errors.subject}</span>
        )}
      </label>
      ...
      <label htmlFor="cf-message">
        ...
        <textarea id="cf-message" name="message" required
          ...
          disabled={status === "loading"} />
        {errors.message ? (
          <span id="cf-message-error" className={styles.fieldError} role="alert">{errors.message}</span>
        ) : (
          <span id="cf-message-hint" style={{ color: "var(--muted)", fontSize: 12 }}>Minimum 20 characters</span>
        )}
      </label>
      ...
      <button className={styles.primaryButton} type="submit"
        ...
        {status === "loading" ? (<><span aria-hidden="true">⏳</span> Sending…</>) : (<>Send Message <b>→</b></>)}
      </button>

      {status === "rate-limited" && (
        <div role="alert" style={{ color: "#d97706", fontSize: 14, fontWeight: 600 }}>
